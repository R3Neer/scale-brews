...
- For entity collisions, read `docs/ENTITY_COLLISIONS_REQUIREMENTS.md`, `docs/ENTITY_COLLISIONS.md`, `docs/ENTITY_COLLISIONS_PLAN.md`, `docs/ENTITY_COLLISIONS_SPRINT_WORKFLOW.md` and `docs/VALIDATION.md` in that order. Do not recreate parallel anatomy/platform plans or status diaries.
- Entity-collision implementation must follow `docs/ENTITY_COLLISIONS_SPRINT_WORKFLOW.md`: select a small dependency-coherent sprint, converge the implementation plan and pre-implementation adversarial model iteratively, implement iteratively, build adversarial tests afterwards, classify every failure before returning to the appropriate phase, and close only after a complete review pass produces zero changes. Sprint documents are execution records, not competing sources of requirements, architecture, roadmap or evidence.
