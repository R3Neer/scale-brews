...
public record ScaleRules(boolean tinyMounts, Map<Identifier, Boolean> mounts, boolean villagerFear,
    ...
    public static final ScaleRules DEFAULT = new ScaleRules(true, Map.of(), true, true,
    ...
    public static final Codec<ScaleRules> CODEC = RecordCodecBuilder.create(i -> i.group(
        ...
        // Decode-only legacy switches: an old disabled component disables the combined impact.
        // Constant default getters omit these obsolete fields when encoding/synchronizing.
        Codec.BOOL.optionalFieldOf("growth_landing_knockback", true).forGetter(r -> true),
        Codec.BOOL.optionalFieldOf("growth_landing_damage", true).forGetter(r -> true),
    ...
    ).apply(i, (tiny, mounts, fear, impact, legacyPush, legacyDamage, environment, farmland, plates, eggs, quiet, terrain) ->
        new ScaleRules(tiny, mounts, fear, impact && legacyPush && legacyDamage, environment, farmland, plates, eggs, quiet, terrain)));
