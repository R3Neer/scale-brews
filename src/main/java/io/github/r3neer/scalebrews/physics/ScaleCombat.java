...
public final class ScaleCombat {
    ...
    public static double knockback(double original, DamageSource source) {
        if (source != null && source.is(LANDING) && source.getEntity() instanceof Player landingPlayer
                && !io.github.r3neer.scalebrews.config.ScaleRules.get(landingPlayer.level()).growthImpactKnockback()) return 0;
