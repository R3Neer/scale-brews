...
/** Differential holdout for the first keyframe boundary in an ordinary multi-keyframe channel. */
public final class S18FirstKeyframeBoundarySemanticClientTests implements FabricClientGameTest {
    private static final String SOURCE = "scalebrews_test:s18_first_keyframe_boundary";
    ...
    public void runTest(ClientGameTestContext context) {
        context.runOnClient(client -> {
            ...
            for (float seconds : new float[]{0f, .499f, .5f, .501f, 1f})
                compareNative(definition, baseline, bound, seconds);

            System.out.println("S18_FIRST_KEYFRAME_BOUNDARY PASS original Minecraft 26.2 multi-keyframe boundary parity");
        });
    }
    ...
    private static void compare(Matrix4f expected, Matrix4f actual, float tolerance, float seconds) {
        ...
        for (int i = 0; i < 16; i++) if (Math.abs(a[i] - b[i]) > tolerance)
            throw new AssertionError("first-keyframe boundary parity seconds=" + seconds + " matrix[" + i + "] expected=" + a[i] + " actual=" + b[i]);
    }
}
