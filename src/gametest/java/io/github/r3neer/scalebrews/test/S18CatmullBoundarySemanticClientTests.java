...
/** Differential holdout for CATMULL_ROM endpoint-neighbor semantics with discontinuous edge keyframes. */
public final class S18CatmullBoundarySemanticClientTests implements FabricClientGameTest {
    private static final String SOURCE = "scalebrews_test:s18_catmull_boundaries";
    ...
    public void runTest(ClientGameTestContext context) {
        context.runOnClient(client -> {
            ...
            for (float seconds : new float[]{.1f, .25f, .75f, 1.25f, 1.75f, 1.999f})
                compareNative(definition, baseline, bound, seconds);

            System.out.println("S18_CATMULL_BOUNDARY PASS original Minecraft 26.2 endpoint-neighbor parity");
        });
    }
    ...
    private static void compare(Matrix4f expected, Matrix4f actual, float tolerance, float seconds) {
        ...
        for (int i = 0; i < 16; i++) if (Math.abs(a[i] - b[i]) > tolerance)
            throw new AssertionError("CATMULL boundary parity seconds=" + seconds + " matrix[" + i + "] expected=" + a[i] + " actual=" + b[i]);
    }
}
