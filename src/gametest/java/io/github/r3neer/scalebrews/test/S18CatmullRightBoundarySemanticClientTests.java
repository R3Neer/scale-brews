...
/** Isolates the right CATMULL_ROM endpoint-neighbor semantics independently from the left edge. */
public final class S18CatmullRightBoundarySemanticClientTests implements FabricClientGameTest {
    private static final String SOURCE = "scalebrews_test:s18_catmull_right_boundary";
        ...
        context.runOnClient(client -> {
            var definition = new AnimationDefinition(2f, false, Map.of("head", List.of(
                new AnimationChannel(AnimationChannel.Targets.POSITION,
                    new Keyframe(0f,
                        KeyframeAnimations.posVec(0, 0, 0),
                        KeyframeAnimations.posVec(0, 0, 0),
                        AnimationChannel.Interpolations.CATMULLROM),
                    new Keyframe(1f,
                        KeyframeAnimations.posVec(8, 0, 0),
                        KeyframeAnimations.posVec(12, 0, 0),
                        AnimationChannel.Interpolations.CATMULLROM),
                    new Keyframe(2f,
                        KeyframeAnimations.posVec(20, 0, 0),
                        KeyframeAnimations.posVec(40, 0, 0),
                        AnimationChannel.Interpolations.CATMULLROM))
            )));
            ...
            // The left endpoint has pre==post, so these late samples isolate the right ghost-neighbor rule.
            for (float seconds : new float[]{1.1f, 1.25f, 1.5f, 1.75f, 1.999f})
                compareNative(definition, baseline, bound, seconds);

            System.out.println("S18_CATMULL_RIGHT_BOUNDARY PASS original Minecraft 26.2 right endpoint-neighbor parity");
        });
    }
    ...
    private static void compare(Matrix4f expected, Matrix4f actual, float tolerance, float seconds) {
        ...
        for (int i = 0; i < 16; i++) if (Math.abs(a[i] - b[i]) > tolerance)
            throw new AssertionError("CATMULL right-boundary parity seconds=" + seconds + " matrix[" + i + "] expected=" + a[i] + " actual=" + b[i]);
    }
}
