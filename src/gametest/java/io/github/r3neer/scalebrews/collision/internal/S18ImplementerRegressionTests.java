...
public final class S18ImplementerRegressionTests {
    ...
    @GameTest
    public void equineMissingAgeScaleUsesSupportedAdultDefault(GameTestHelper h) {
        ...
        var baseChannels = Map.of("eat", 0f, "stand", 0f, "mouth", 0f, "tail", 0f, "water", 0f);
        var omitted = equine.evaluate(geometry,
            new PoseEngine.Inputs(1, .5f, 10, 0, 0, true, baseChannels), Map.of()).orElseThrow();
        var explicit = equine.evaluate(geometry,
            new PoseEngine.Inputs(1, .5f, 10, 0, 0, true,
                Map.of("eat", 0f, "stand", 0f, "mouth", 0f, "tail", 0f, "water", 0f, "age_scale", 1f)),
            Map.of()).orElseThrow();
        h.assertTrue(omitted.keySet().equals(explicit.keySet()),
            "Omitted adult age_scale and explicit age_scale=1 must animate the same equine parts");
        for (String part : omitted.keySet())
            assertMatrixNear(h, explicit.get(part), omitted.get(part),
                "Supported ordinary adult equines default age_scale to one: " + part);
        h.succeed();
    }
