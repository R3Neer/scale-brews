...
**Prioridad G4 vigente:** **G4.1 / S25 sigue ABIERTO por RED productivo de identidad reference↔receipt.** La captura client-side y el orden wire ya están reparados: el dedicated proof emite anatomy references inmediatamente antes de movement vanilla y no recibe correcciones. El owner/kernel está verde con **10 mutantes compilables muertos** (exactly-once, tracking generation, lifecycle, receipt TTL, saturation, non-controller, double-apply, fabricated-hit, rate budget y pending TTL). El blocker actual es la clave causal: en player RTT=0 el cliente envía `supportFrameSerial` pares `8,10,...,404`, mientras los receipts server-side vivos están ligados a seriales impares `335,337,...,403`; la intersección es vacía y `consumedReferences=0`. Handoff: `docs/sprints/S25-adversarial-receipt-frame-identity-red.md`. **No se inicia G4.2** hasta que la referencia nombre una identidad estable del transporte incorporado y player + controlled boat consuman receipts reales a 0/100/200 ms.
