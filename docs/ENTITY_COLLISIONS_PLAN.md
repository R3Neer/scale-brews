...
**Prioridad G4 vigente:** **G4.1 / S25 sigue ABIERTO por RED productivo de identidad reference↔receipt.** La captura client-side y el orden wire ya están reparados: el dedicated proof emite anatomy references inmediatamente antes de movement vanilla y no recibe correcciones. El owner/kernel está cerrado adversarialmente en run `35451920466`: baseline + **14 mutantes compilables muertos**, incluidos epoch/dimension/network-id separados y un mutante `adjacent-frame-match` que acepta `±1`. Ordinary `35451920485`, job `105920454100`: **446/446**. El blocker actual sigue siendo la clave causal: en player RTT=0 el cliente envía `supportFrameSerial` pares `8,10,...,404`, mientras los receipts server-side vivos están ligados a seriales impares `335,337,...,403`; la intersección es vacía y `consumedReferences=0`. No vale reparar por proximidad/paridad/nearest-frame, y `rootFrameSequence` solo también es ambiguo para contribuciones joint distintas. Handoff: `docs/sprints/S25-adversarial-receipt-frame-identity-red.md`. **No se inicia G4.2** hasta que la referencia nombre una identidad server-issued estable del transporte incorporado y player + controlled boat consuman receipts reales a 0/100/200 ms.
