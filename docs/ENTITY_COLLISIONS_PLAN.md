...
**Prioridad G4 vigente:** **G4.1 tiene candidato IMPLEMENTER y sigue ABIERTO pendiente de cierre ADVERSARY.** `AnatomyMoveReferencePayload` aporta una ruta C2S metadata-only que nombra un endpoint causal ya publicado por el servidor mediante `support UUID + supportFrameSerial`; el servidor deriva player/controlled vehicle desde la conexión, resuelve únicamente receipts server-issued recipient/body-local, conserva el receipt histórico y guarda exactly-once como metadata de consumo. El rebase usa sólo `TransportLedger.since(receipt.transportSequence())`, por lo que no reaplica `receipt.appliedDelta`. El candidato productivo `b09c490a8405cf5042468e8ea1bd6d4538fad0cd` pasó build `35388973833`, presence `35388973889`, authority `35388973830`, receipt isolation `35388973790` y tracking ledger `35388973836`. La regresión implementer posterior `35224ed5261969854feb58379f2fcd34927d61c9` pasó `s25-implementer-reference-proof` run `35444126698` y build `35444126655`. **No se inicia G4.2** hasta que el adversario ejecute A–G y los ocho mutation-kills de S25 sobre el candidato vigente.
