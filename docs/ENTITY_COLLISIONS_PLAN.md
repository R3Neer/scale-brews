...
**Estado:** **ABIERTO / en ejecución**. S05-S13 están cerrados. La física material Q2 está cerrada y probada en ordinary + prepared real-geometry: broadphase acotado, identidad causal de intervalos, dispatcher ROOT/JOINT, transporte anclado y `DERIVED_CARRY`, multicontacto/sliding/recovery, wall squeeze, contacto sólo intermedio, budgets exactos y precisión estable bajo traslaciones mundiales grandes. S10 extrajo el passive transport ledger/cursor, S11 eliminó la doble autoridad de gravedad, S12 extrajo el ownership persistente de contacto/anchor/suspensión y S13 extrajo el índice espacial bounded. G2 permanece abierto exclusivamente por la partición/ownership arquitectónicos restantes de las tareas 1 y 9.
...
**S09 cerrado tras reaperturas adversariales tardías:** multicontacto/sliding/recovery quedó cubierto por tangential retention, floor+wall y triple contacto, separación inicial acotada, wall squeeze/localidad, contacto estrictamente intermedio y metamorfismos de traslación/gravedad. La banda numérica de contacto quedó unificada entre CCD y screening; el scheduler prioriza trabajo causal sin omitir piezas; y la reapertura prepared durante S13 eliminó una doble carga en `validCorrection(q)` manteniendo route-check y `SKIN+ULP` final con una sola muestra presupuestada por pieza. El último cambio productivo propio es `de44c78c75bcc54ef423af783d35761014b49356`: ordinary run `34695839944`, job `103559056801`, **380/380**, artifact `10299110544`, SHA-256 `58c33a23d8a8cbd9256736e56d55ffeee43b93cb7b02dcf5df2e8d78d1b7985b`; prepared run `34695839860`, job `103559056605`, export original verde y servidor **2/2**. `a1f1a568726d95664c6e8b4a144659c36be0e5cc` endureció después A9 con varias traslaciones mundiales explícitas; prepared run `34696004864`, job `103559482598`, volvió a quedar **2/2** sin cambiar producción ni budgets.

**S10 cerrado:** `docs/sprints/S10-passive-transport-ledger.md` extrajo a `collision.runtime.TransportLedger` el ownership único de current/history/generation/cursor de transporte pasivo. `AuthorityPoseTracker` consume el ledger directamente; `AnatomyMovement` conserva únicamente la aplicación física del carry y las façades con consumidores reales. La revisión final retiró la façade de cursor que quedó huérfana tras la migración. S10 cubre una subresponsabilidad de las tareas 1/9 y no cierra G2 por sí solo.
...
S09 cerró las tareas físicas 5, 7 y 8 y soportó varias reaperturas adversariales posteriores: aliases/recovery, screening temporal preparado, tolerancia numérica a coordenadas mundiales grandes, divergencia entre la banda ULP de CCD y los certificados temporales, scheduling causal y finalmente doble carga del budget durante `q`. Ninguna reapertura elevó budgets ni cambió requisitos. El último cambio productivo es `de44c78...`; `a1f1a568...` fija además A9 bajo varias traslaciones mundiales explícitas y mantiene prepared **2/2**.
...
S13 extrajo el índice espacial per-level/tick a `AnatomySpatialIndex`, eliminó `FrameStamp/frames` muertos y fijó locality, overflow, lifecycle, single-sample rebuild y stale-caller fail-closed. `3cd0209...` es su último cambio productivo propio. Su prepared post-hardening reabrió de forma independiente A9/S09; tras la reparación S09, la cadena integrada `a1f1a568...` queda ordinary **380/380** y prepared **2/2**.

Después de S13, G2 continúa únicamente con las tareas arquitectónicas 1 y 9. El siguiente sprint no se numera ni se define hasta auditar el ownership restante de `AnatomyMovement` y demostrar una frontera separable con consumidores reales, dependencias acíclicas y holdouts propios.

### Revisión del código

El inventario y revisiones destructivas de G0 fijaron qué conservar/eliminar. G1 dejó fronteras públicas/data estables. S05 extrajo `MaterialBroadphase`; S06-S09 establecieron fronteras causales y físicas reales alrededor de `MaterialIntervalRuntime`, `MaterialPhysicsRuntime`, `AnchoredTransportPlanner`, `TemporalResponse`, `AnatomySeparation` y el dispatcher. S10 extrajo `TransportLedger`; S11 retiró la doble autoridad de gravedad; S12 extrajo el estado retenido de contacto a `AnatomyContactState`; S13 extrajo membership/index bounded a `AnatomySpatialIndex` y eliminó metadata espacial causal que no tenía consumidor.

`AnatomyMovement` sigue sin considerarse completamente particionado. Provider/binding/endpoint authority permanece como el bloque persistente más claro a auditar, pero su separación sólo es válida si no introduce callbacks inversos hacia sampling/root/spatial policy. Root history permanece deliberadamente por ahora porque su identidad está entrelazada con causal endpoints y la generalización de `RootTransformProvider` pertenece a G3. El siguiente corte sólo moverá ownership cuando exista una frontera verificable con consumidores reales; la extracción por estética sigue prohibida.
