...
**Prioridad G4 vigente:** **G4.1 / S25 sigue ABIERTO por RED productivo de integración cliente.** El kernel reference/receipt ya está verde y mutation-adequate con nueve mutantes compilables muertos: exactly-once, tracking generation, lifecycle identity, expired receipt, saturated tick, non-controller, double-apply, fabricated-hit y rate budget. El dedicated proof alcanza player RTT=0 con soporte real, READY/local authority/capability válidos y `TransportLedger` cliente creciente, pero no emite ningún `anatomy_move_reference_v1`: el cursor se intenta capturar en END_CLIENT_TICK después de que la ruta real `Entity.move -> PlatformEntityMixin -> PlatformPhysics.carry -> AnatomyMovement.carry` ya avanzara el ledger. Handoff: `docs/sprints/S25-adversarial-latency-capture-red.md`. **No se inicia G4.2** hasta reparar y recertificar player + controlled boat a 0/100/200 ms.
