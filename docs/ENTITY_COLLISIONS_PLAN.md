...
**Prioridad G4 vigente:** **G4.1 / S25 sigue ABIERTO por RED productivo de identidad reference↔receipt.** La captura client-side y el orden wire ya están reparados: el dedicated proof emite anatomy references inmediatamente antes de movement vanilla y no recibe correcciones. El owner/kernel está cerrado adversarialmente en run `35452742190`: baseline + **15 mutantes compilables muertos**, incluidos epoch/dimension/network-id separados, `adjacent-frame-match` (`±1`) y `ambiguous-exact-match` (elegir arbitrariamente entre dos receipts con la misma identidad). Ordinary `35452742176`, job `105922622233`: **446/446**. La frontera de schema mata además un payload C2S que suba `localTransportSequence` cliente en run `35452660678`. El blocker actual sigue siendo la clave causal: en player RTT=0 el cliente envía `supportFrameSerial` pares `8,10,...,404`, mientras los receipts server-side vivos están ligados a seriales impares `335,337,...,403`; la intersección es vacía y `consumedReferences=0`. No vale reparar por proximidad/paridad/nearest-frame; support/tick/contact/root tampoco identifican de forma única un transporte, y `rootFrameSequence` solo es ambiguo para contribuciones joint distintas. Handoff: `docs/sprints/S25-adversarial-receipt-frame-identity-red.md`. **No se inicia G4.2** hasta que la referencia nombre una identidad server-issued, exacta y única del transporte incorporado y player + controlled boat consuman receipts reales a 0/100/200 ms.
