...
**Prioridad G4 vigente:** implementar task **1** con un reference metadata-only que sólo pueda nombrar receipts server-issued, exactly-once/TTL/lifecycle fenced y sin crear geometry/contact/carry/tracking authority. La presence gate es temporal y debe sustituirse por holdouts conductuales cuando exista la superficie real. La frontera previa `s25-adversarial-reference-authority` está verde/mutation-sensitive en run `35384438639`: mata borrow legacy common/server, borrow legacy client-side y un payload serverbound de autoridad física rica; ordinary `35384438584` permanece verde. El store server-side queda además fijado por `s25-adversarial-receipt-isolation` run `35384233437`: recipient key, body UUID, disconnect y saturation scoped están cubiertos con tres mutantes compilables muertos.
