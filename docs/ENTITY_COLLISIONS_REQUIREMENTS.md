...
La colisión anatómica tampoco sustituye el **empuje vanilla entre entidades**. Una entidad que no califica como moving platform/soporte material para un cuerpo no adquiere frente a él una pared anatómica adicional: la pareja conserva la colisión/empuje vanilla ordinarios. Cuando sí existe una relación soporte-cuerpo válida, Scale añade soporte, contacto material y carry, pero el efecto de empuje vanilla de la entidad sobre el cuerpo debe seguir existiendo exactamente una vez; la integración no puede conseguir estabilidad convirtiendo el soporte en una pared AABB ni anulando silenciosamente ese push.
...
| FR-053 | La anatomía sólo PUEDE sustituir el **bloqueo sólido/AABB** de la pareja soporte-cuerpo mientras exista una relación moving-platform válida. Si la pareja no es elegible por ratio/policy/estado, NO DEBE aparecer una pared anatómica y rige la colisión vanilla ordinaria. Incluso durante una relación soporte-cuerpo válida, el efecto de `push` vanilla de la entidad soporte sobre el cuerpo DEBE conservarse exactamente una vez; no puede desaparecer como efecto colateral de suprimir el bloqueo AABB ni duplicarse por el solver anatómico. | Tests `<`, `==` y `>` del ratio y policy off/on demuestran: no-platform = sin pared anatómica + push vanilla; platform = soporte/carry anatómico + mismo push neto vanilla exactamente una vez. Terceras entidades mantienen su comportamiento vanilla. |
...
| FR-094 | Scale DEBE poder publicar sus contactos/sweeps materiales válidos a la extensión pública de **landing surfaces** de Clinging Reoriented (actualmente `LandingSurfaceProvider`/`LandingSurfaces` o capability equivalente), con identidad/revisión suficientes para `currentSupport`, `sweep` y `revalidate`. Ese adapter sólo declara geometría/contacto válido: Clinging conserva ownership de predicción de aterrizaje, decisión de Reorientation, giro, reposicionamiento y presentación. | Consumer integration registra un provider Scale, un contacto anatómico elegible puede comprometer/revalidar un landing de Reorientation, una entidad no elegible no aparece como landing surface, y no existe un segundo solver/carry en Clinging. |
...
| FR-093 | Una entidad sólo PUEDE actuar como pared/superficie anatómica frente a un cuerpo cuando esa pareja califica realmente como moving platform/soporte material según ratio, policy y estado. Fuera de esa elegibilidad, la pareja conserva únicamente su comportamiento físico vanilla entre entidades. Dentro de la elegibilidad, Scale puede sustituir el bloqueo sólido global por geometría anatómica, pero DEBE conservar el empuje vanilla de la entidad soporte sobre el cuerpo exactamente una vez. | Casos por debajo, en y por encima del ratio demuestran ausencia de pared anatómica cuando no hay plataforma; los casos platform conservan soporte/carry anatómico y el mismo desplazamiento/impulso de push vanilla que el control, sin doble aplicación. |
...
| ID | Requisito no funcional | Criterio mínimo de aceptación |
| --- | --- | --- |
| NFR-033 | Activar el subsistema NO DEBE alterar colisión/pathfinding global, combate, interacción o física de entidades que no participan en un contacto gestionado. Para la propia pareja soporte-cuerpo, FR-053/FR-093 preservan además el push vanilla exactamente una vez aunque Scale sustituya el bloqueo sólido por anatomía. | Comparativas vanilla/feature para terceros y entidades fuera de policy; fixtures platform/non-platform comparan explícitamente el desplazamiento/impulso de push frente al control vanilla. |
