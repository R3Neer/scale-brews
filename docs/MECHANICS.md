...
Villager size fear compares both entities' effective SCALE, rather than requiring a player at Growth II. Map SCALE to a continuous position: `(scale - 1) / 0.96` at or above normal, `(scale - 1) / 0.242` below normal. A visible living entity within eight blocks is an additional threat when its position exceeds the villager's by at least two (with a 0.000001 rounding tolerance). For example, Shrinking I (-1) fears Growth I (+1), normal (0) fears Growth II (+2), and Shrinking II (-2) fears normal (0). Positions remain uncapped for externally enlarged entities. This compares scale modifiers, not species' natural heights. Vanilla visibility, panic memory, hostile threats and reputation rules remain intact.

For example, Growth III + Shrinking III targets `3.88 * 0.274 = 1.06312`: this grants about 6.6% of Growth I's continuous changes, not Growth III abilities. No landing wave is possible at that size. Scale 1 has no derived bonus or penalty, regardless of how it was obtained. Attribute health changes preserve the health fraction at each transition step. No size-dependent changes are applied to natural regeneration costs.
...
`ScaleExhaustion` multiplies only physical costs: Growth 1.10/1.20/1.30 and Shrinking .95/.90/.85. Costs interpolate with effective size; opposing effects do not apply two competing cost factors. `ScaleMovementExhaustionMixin` targets ServerPlayer movement statistics (sprint, swimming and water movement) and both ground-jump branches. `ScaleAttackExhaustionMixin` targets Player attack and the 26.2 stabbing attack. Ordinary walking's zero exhaustion remains zero. The global `causeFoodExhaustion` / `FoodData` sink is untouched, so regeneration, Hunger, received damage, mining and unrelated sources retain vanilla costs. Food values and hunger/saturation are never directly edited by the mod. Custom attacks from mods bypassing vanilla call sites are not automatically covered.

`ScaleSize` snapshots and restores the health fraction around each physical-size-driven attribute update. `ScaleHealthMixin` also retains lifecycle snapshots for integration. This fixes the tutorial event handler, which missed some power changes. Each invocation owns its snapshot; no global entity map is retained.
...
Impact radius is `min(6, 1.5 + tier*0.65 + min(20, fall-3)*0.08)`. Knockback strength is `min(2, (0.2 + tier*0.2)*(1 + min(20, fall-3)*0.06))`, multiplied by linear path-distance falloff and the target's vanilla knockback resistance. Targets must stand on a connected collision surface reachable within the path-length budget. The bounded search uses exposed collision tops, permits up to one block of ascent or descent per connection, and counts the three-dimensional connection length plus endpoint offsets. It can follow several stairs or turn around obstacles; gaps, tall ledges, closed barriers and diagonal corner cutting block propagation. Slabs use their actual collision height. Unloaded chunks are not generated by this search. Allies and PvP-disabled players are excluded. No blocks are broken, no fire or explosions are created.
