...
Potion levels only determine the scale target. All derived attributes and physical rules read the current `Attributes.SCALE`, including the blend and external modifiers. For scale `s >= 1`, Growth-equivalent strength is `clamp((s - 1) / 0.96, 0, 3)`; below 1, Shrinking-equivalent strength is `clamp((1 - s) / 0.24, 0, 3)`. Only one family can apply at once. Values beyond the level-III anchors keep the bounded level-III balance.

Continuous properties interpolate between normal and the existing I/II/III anchors: movement, sprint, health, damage, reach, resistance, jump, terrain relief, exhaustion, melee knockback, fall protection, vibration radius, Swift Sneak synergy and creeper power. Discrete abilities use reached size thresholds: villager fear at >= 2.92; insensitive plates at <= 0.52; all plates and fragile-block protection at <= 0.28. Landing knockback requires >= 1.96 and damage >= 3.88, independently of active potion names. Mounting and platform eligibility retain their actual-dimension policies.

For example, Growth III + Shrinking III targets `3.88 * 0.28 = 1.0864`: this grants only 9% of Growth I's continuous changes, not Growth III abilities. No landing wave is possible at that size. Scale 1 has no derived bonus or penalty, regardless of how it was obtained. Attribute health changes preserve the health fraction at each transition step. No size-dependent changes are applied to natural regeneration costs.
